...
export default async function AdminPage() {
  ...
  const [{ data: restaurants }, { data: staff }, { data: admins }, { data: profiles }, users] =
    await Promise.all([
      db.from("restaurants").select("id, name, logo, owner_id, created_at").order("created_at"),
  ...
  const userRows: AdminUserRow[] = users.data.users
    .map((u) => {
      ...
      const restaurant = founded ?? (membership ? restaurantById.get(membership.restaurant_id) : undefined);
      return {
        user_id: u.id,
        email: u.email ?? "—",
